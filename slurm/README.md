...
By default, `DATA_ROOT` is `/gpfs/data/oermannlab/public_data/continual_learning` (bigpurple shared data). `run_benchmarks.sh` runs `prepare_data.py` when ImageNet-R / ImageNet-A train–test splits are missing. Some datasets (e.g. CUB-200, Omnibenchmark) must be laid out manually under that tree; see comments at the top of `run_benchmarks.sh`. Override the location with `DATA_ROOT` if your data live elsewhere.
...
| Variable | Purpose |
|----------|---------|
| `DATA_ROOT` | Dataset root (default `/gpfs/data/oermannlab/public_data/continual_learning` on bigpurple) |
